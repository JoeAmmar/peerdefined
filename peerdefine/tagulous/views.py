...
def autocomplete(request, tag_model):
    ...
    if query:
        ...
        if options.case_sensitive:
            ...
            resultsTerms = terms.filter(name__startswith=query)
        else:
            results = queryset.filter(name__istartswith=query)
            resultsTerms = terms.filter(name__istartswith=query)
    else:
        results = queryset.all()
        resultsTerms = queryset.all()
    ...
    if options.autocomplete_limit:
        ...
        if(str(results.model) != "<class 'definitions.models.Discipline'>"):
            resultsAll = list(set([tag.name for tag in results] + [term.name for term in resultsTerms]))
        else:
            resultsAll = list(set([tag.name for tag in results]))
        more = len(resultsAll) > end
        resultsAll = resultsAll[start:end]
    else:
        if(str(results.model) != "<class 'definitions.models.Discipline'>"):
            resultsAll = list(set([tag.name for tag in results] + [term.name for term in resultsTerms]))
        else:
